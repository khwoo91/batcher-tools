...
export class FileQueue extends LitElement {
  ...
  protected override render() {
    ...
    return html`
      <div
        class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col h-100"
      >
        <div
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="text-md font-semibold text-white font-sans"
                >파일 대기열 (${this.svgFiles.length}개)</span
              >
            </div>
          </div>
          <span class="text-xs text-slate-500 font-sans font-medium">상태 모니터</span>
        </div>
                ...
                (file) => html`
                  ...
                  >
                    <div class="flex items-center gap-3 min-w-0 font-sans flex-1">
                      ...
                      <i
                        class="fa-regular fa-file-image ${file.selected ? 'text-indigo-400' : 'text-slate-600'} text-lg shrink-0 transition-colors"
                      ></i>
                      <div class="min-w-0 flex-1">
                        <p
                          class="font-medium ${file.selected ? 'text-slate-200' : 'text-slate-500 line-through decoration-slate-700'} truncate"
                    ...
                    <div class="flex items-center gap-3 shrink-0 ml-3">
                      ...
                      <div>
                        ${file.status === "pending"
                          ? html`
                              <span
                                class="px-2.5 py-1 ${file.selected ? 'bg-slate-900 text-slate-400 border-slate-800' : 'bg-slate-950 text-slate-600 border-slate-900'} border rounded-full text-[10px] font-semibold transition-colors"
                                >대기 중</span
                              >
                            `
                          : ""}
                        ${file.status === "processing"
                          ? html`
                              <span
                                class="px-2.5 py-1 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full text-[10px] font-semibold animate-pulse"
                                >렌더링 중</span
                              >
                            `
                          : ""}
                        ${file.status === "success"
                          ? html`
                              <span
                                class="px-2.5 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full text-[10px] font-semibold flex items-center gap-1 font-sans"
                                >
                        ...
                        ${file.status === "error"
                          ? html`
                              <span
                                class="px-2.5 py-1 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-full text-[10px] font-semibold flex items-center gap-1"
                                title="${file.errorMsg || ""}"
                              >
