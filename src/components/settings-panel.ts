...
export class SettingsPanel extends LitElement {
  ...
  private handleStartConversion() {
    this.dispatchEvent(new CustomEvent('start-conversion', { bubbles: true, composed: true }));
  }
      ...
      <div class="space-y-6">
        ...
        >
          ${this.isConverting ? html`
            ...
            <span>변환 프로세스 동작 중... (${this.conversionProgress}%)</span>
          ` : html`
            <i class="fa-solid fa-play"></i>
            <span>배치 ${this.exportFormat.toUpperCase()} 변환 작업 시작</span>
          `}
        </button>
      </div>
    `;
