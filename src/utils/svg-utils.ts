 ...
 */
export async function getSvgDimensions(file: File): Promise<{ width: number; height: number; text: string }> {
...
export async function convertSvgToImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => {
      URL.revokeObjectURL(url);
      reject(new Error('SVG 이미지 로드 실패 (정의되지 않은 태그나 유효하지 않은 XML 구문 가능성)'));
    };
